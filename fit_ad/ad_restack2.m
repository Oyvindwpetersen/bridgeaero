function [AD_stiff,AD_damp]=ad_restack2(AD)
%% Convert ADs: from single vector to 3*3 (stiffness and damping)
%
% Inputs:
% AD: 18*N matrix
%
...
AD_stiff=zeros(3,3,size(AD,2));
AD_damp=zeros(3,3,size(AD,2));
for k1=1:3
    for k2=1:3

        AD_stiff(k1,k2,:)=AD(idx_s(k1,k2),:);
        AD_damp(k1,k2,:)=AD(idx_d(k1,k2),:);
